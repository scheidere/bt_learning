...
def run():
    ...
    # Create output file
    # f = open("/home/graeme/Dropbox/emily_graeme_shared/results_from_graeme/simple/all_methods_output/n_3/" + str(start_time_milli) + current_method + "_output.txt","w+") #overall output file, can't load while running
    ...
    simulator = SimpleSimulator(seed=seed)
    ...
    exploration_exploitation_parameter = 0.75 #config["exploration_exploitation_parameter"]
    max_mcts_iterations = 50 #config["max_mcts_iterations"]
    ...
    # overall_best_word, overall_best_word_score, total_time_to_best, num_rounds_to_best, total_time_for_run, best_reward_per_round_list = all_methods.run(cfg, budget, exploration_exploitation_parameter, max_sim_iterations, simulator, use_dag, config)



    [solution, best_rollout, root, list_of_all_nodes, winner, best_rollout_node, best_nodes_dict, best_reward] = mcts( cfg, budget, max_mcts_iterations, exploration_exploitation_parameter, max_sim_iterations, simulator, use_dag, config, [] )
