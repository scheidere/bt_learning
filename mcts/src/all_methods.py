...
class AllMethods():
    ...
    def run(self, cfg, budget, exploration_exploitation_parameter, max_sim_iterations, underwater_simulator, use_dag, config):
        ...
        f = open("/home/graeme/Dropbox/emily_graeme_shared/results_from_graeme/simple/intermediate_output_files/n_3/" + str(start_time) + current_method + "_output.txt","w+") #overall output file, can't load while running
        #print(f.read())

        # Do the rounds
        for round in xrange(self.num_rounds):

            f1 = open("/home/graeme/Dropbox/emily_graeme_shared/results_from_graeme/simple/intermediate_output_files/n_3/" + str(start_time) + current_method + "_output_thru_round" + str(round) + ".txt","w+")
            ...
            if overall_best_word != None:
                ...
                f1.write("overall_best_word is None\n")
